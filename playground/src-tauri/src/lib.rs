...
#[cfg(desktop)]
fn configure_menus(app: &mut tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app_menu = SubmenuBuilder::new(app, &app_name)
        .item(&PredefinedMenuItem::about(
            app,
            Some("About Divvun Runtime Playground".into()),
            None,
        )?)
        .separator()
        .item(&PredefinedMenuItem::hide(
            app,
            Some("Hide Divvun Runtime Playground".into()),
        )?)
    ...
    app.on_menu_event(move |app, event| {
        ...
        match event.id().as_ref() {
            ...
            "close_tab" => {
                if let Some(window) = &window {
                    let _ = window.eval("window.dispatchEvent(new CustomEvent('menu-close-tab'))");
                }
            }
            "duplicate_tab" => {
                if let Some(window) = &window {
                    let _ =
                        window.eval("window.dispatchEvent(new CustomEvent('menu-duplicate-tab'))");
                }
            }
            _ => {}
        }
